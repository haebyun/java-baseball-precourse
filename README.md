...
### 게임 흐름
- [ ] 게임 시작 시 컴퓨터가 1~9 범위의 서로 다른 숫자 3개를 생성한다.
- [ ] 사용자에게 숫자 입력을 요청하고 입력을 받는다.
- [ ] 입력한 숫자에 대한 스트라이크/볼/낫싱 힌트를 계산하여 출력한다.
- [ ] 3스트라이크가 되면 게임 종료 메시지를 출력한다.
- [ ] 게임 종료 후 `1(재시작)` / `2(종료)` 입력을 받아 처리한다.
    - [ ] 재시작 시 정답 숫자를 새로 생성하여 게임을 다시 진행한다.
    - [ ] 종료를 선택하면 프로그램을 종료한다.
...
- [ ] 숫자 입력 검증
    - [ ] 3자리인지 검증한다.
    - [ ] 각 자리가 1~9 범위인지 검증한다. (도메인: `BaseballNumber`)
    - [ ] 3자리 숫자가 서로 다른 수인지 검증한다. (도메인: `BaseballNumbers`)
- [ ] 재시작/종료 입력 검증
    - [ ] 1 또는 2만 허용한다.
